    ...
    <div className="space-y-6">
      <div className="card-midnight p-6 border border-midnight flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-2xl font-bold text-primary mb-1">Security Report</h2>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <div className="bg-tertiary rounded-xl p-4 text-sm border border-midnight shadow-sm">
                      <div className="flex items-center gap-2 mb-1.5 text-coral/90">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        <span className="font-bold text-xs uppercase tracking-wider">Recommendation</span>
                      </div>
                      <span className="text-muted leading-relaxed">{finding.recommendation}</span>
                    </div>

                    {finding.howToFix && (
                      <div className="bg-black/60 rounded-xl p-4 text-sm border border-midnight/50 shadow-inner">
                        <div className="flex items-center gap-2 mb-2 text-emerald-400/90">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
                          <span className="font-bold text-xs uppercase tracking-wider">How to Fix</span>
                        </div>
                        <div className="text-gray-300 font-mono text-[13px] leading-relaxed whitespace-pre-wrap">{finding.howToFix}</div>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="flex justify-end">
        <button onClick={runScan} className="text-sm text-muted hover:text-coral transition-colors flex items-center gap-1.5">
