...
export default function Repositories() {
  ...
  useEffect(() => {
    ...
    const savedState = sessionStorage.getItem('repoSearchState')
    if (savedState) {
      try {
        const state = JSON.parse(savedState)
        setSearch(state.search || '')
        setSearchType(state.searchType || 'repo')
        setMinStars(state.minStars || '')
        setMinForks(state.minForks || '')
        setSortBy(state.sortBy || 'stars')
        setOrder(state.order || 'desc')
  ...
  const fetchRepos = async () => {
    try {
      ...
      sessionStorage.setItem('repoSearchState', JSON.stringify({
        search, searchType, minStars, minForks, sortBy, order, page
      }))
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div className="flex-1 w-full sm:w-auto">
                      <div className="flex flex-wrap items-center gap-3 mb-2">
                        ...
                        <AIHealthBadge owner={repo.owner.login} repo={repo.name} />
                      </div>
                      <p className="text-muted mb-4">{repo.description || 'No description'}</p>
                    ...
                    <button
                      ...
                      title={bookmarkedIds.has(repo.id) ? "Remove Bookmark" : "Add Bookmark"}
                      className={`w-10 h-10 rounded-full flex items-center justify-center transition-all hover:scale-110 ${
                        bookmarkedIds.has(repo.id)
                          ? 'bg-coral text-white shadow-[0_0_15px_rgba(255,107,107,0.4)]'
                          : 'bg-tertiary border border-midnight text-muted hover:text-coral hover:border-coral'
                      }`}
                    >
                      {bookmarkedIds.has(repo.id) ? (
